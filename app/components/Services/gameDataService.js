wciApp.factory(
    ...
    *  Which will add other bonuses to the formula, like research or terrain type(if we ever implement that)*/
		...
		data.init = function (excelObject) {
			...
			for (const key in excelObject) {
				if (excelObject.hasOwnProperty(key)) {
					...
					for (const value of convertToObject) if (value === key) this[key] = this.convertToObject(excelObject[key]);

					for (const value of removeUndefined) if (value === key) this[key] = this.removeUndefined(excelObject[key]);
		...
		data.removeUndefined = function (excelObject) {
			for (const [ index, array ] of excelObject.entries()) for (const [ key, property ] of Object.entries(array)) if (property === undefined) delete excelObject[index][key];
		...
		data.convertToObject = function (excelObject) {
			...
			for (const value of excelObject) object[value.id] = value.name;
